...
REM 變數設定=====
set ALL_SETTING=.\Update_Set_GetItem.txt 
@Echo Off
...
echo "也就是會從[ %FROM_FOLDER% ]複製位置到[ %GOAL_FOLDER%\%yyyymmdd% ]"
...
echo "包版目標位置[ %GOAL_FOLDER% ]"
CHOICE /C YNC /M "確認請按 Y，否請按 N，或者取消請按 C。" 

echo "複製包版LIST:"
XCOPY "%LIST%" "%GOAL_FOLDER%\%yyyymmdd%" /S /D /Y /C /R  

echo "開始執行複製:"
for /f %%a in (%LIST%) do ( 
    for %%F in ("%%a") do XCOPY "%FROM_FOLDER%%%a" "%GOAL_FOLDER%\%yyyymmdd%%%~pF" /S /D /Y /C /R   
)
